/**
* The central orchestration component for the Latent Model Organizer.
*
* This component implements a high-end desktop experience using Vue 3 and Electron.
* It features a glassmorphic interface with a custom draggable titlebar, tabbed
* navigation between the Sorter and Fetcher modules, and deep integration with
* the backend REST API.
*
* Key Architectural Features:
* - Dynamic Theme Engine: Supports multiple visual profiles (Neon, Gold, Fanfiction)
*   synchronized via CSS variables.
* - Reactive State Persistence: Automatically mirrors user preferences, folder paths,
*   and configuration flags to localStorage.
* - Integrated Tooling: Houses the console drawer for real-time log streaming and
*   specialized modals for settings and operation reports.
* - Multi-stage Workflow: Manages directory selection, architecture filtering,
*   and asynchronous API execution with robust error handling.
*/
<script setup>
...
const THEMES = ['neon', 'gold', 'light', 'fanfriction', 'fanfriction-light'];
const currentTheme = ref(localStorage.getItem('lmo:theme') || 'neon');
const applyTheme = (name) => {
  currentTheme.value = name;
  document.body.className = '';
  document.body.classList.add(`theme-${name}`);
  localStorage.setItem('lmo:theme', name);
};
...
const executeOrganization = () => {
  ...
  if (!selectedArchitectures.value.length) {
...
<template>
  <div class="layout-wrapper">
    ...
    <header class="menubar-glass draggable-header">
      ...
      <div class="hdr-logo no-drag">
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" class="logo-icon">
      ...
      <nav class="hdr-nav no-drag">
        <button class="nav-btn" :class="{ 'active-nav-btn': activeTab === 'sort'  }" @click="activeTab = 'sort'"
                :disabled="isProcessing">
          <i class="pi pi-sort-alt"></i> Sorter
        </button>
        <button class="nav-btn" :class="{ 'active-nav-btn': activeTab === 'fetch' }" @click="activeTab = 'fetch'"
                :disabled="isProcessing">
          <i class="pi pi-cloud-download"></i> Fetcher
        </button>
      </nav>

      <div class="hdr-controls no-drag">
        <button class="nav-btn icon-only" :class="{ 'active-nav-btn': showSettings }" @click="showSettings = true"
                title="Settings"><i class="pi pi-cog"></i></button>
        <button class="nav-btn icon-only" :class="{ 'active-nav-btn': consoleOpen  }"
                @click="consoleOpen = !consoleOpen" title="Toggle Console"><i class="pi pi-terminal"></i></button>
        <button class="nav-btn icon-only" @click="minimizeWindow" title="Minimize"><i class="pi pi-minus"></i></button>
        <button class="nav-btn icon-only" @click="maximizeWindow" title="Maximize"><i class="pi pi-window-maximize"></i>
        </button>
        <button class="nav-btn icon-only window-close-btn" @click="closeWindow" title="Close"><i
          ...
          <div class="glass-panel main-card">
            ...
            <div v-if="activeTab === 'sort'" class="tab-content">
              ...
              <div class="form-group">
                ...
                <div class="input-row">
                  <input class="glass-input" type="text" readonly :value="sourceFolder"
                         placeholder="Select source folder…"/>
                  <button class="secondary-btn" @click="pickSource" :disabled="isProcessing"><i
                      class="pi pi-folder-open"></i> Browse
                  </button>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="input-row">
                  <input class="glass-input" type="text" readonly :value="targetFolder"
                         placeholder="Select target folder…"/>
                  <button class="secondary-btn" @click="pickTarget" :disabled="isProcessing"><i
                      class="pi pi-folder-open"></i> Browse
                  </button>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="arch-dropdown" ref="dropdownRef">
                  <button class="arch-trigger glass-input" :class="{ open: dropdownOpen, 'is-none': noneSelected }"
                    ...
                    <span class="arch-trigger-badge"
                          v-if="!allSelected && !noneSelected">{{ selectedArchitectures.length }}</span>
                  ...
                  <div v-if="dropdownOpen" class="arch-panel glass-panel">
                    ...
                    <ul class="arch-list">
                      <li v-for="arch in filteredArchs" :key="arch" class="arch-item"
                          :class="{ selected: isSelected(arch) }" @click="toggleArch(arch)">
                        <span class="arch-check">
                          <svg v-if="isSelected(arch)" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"
                               fill="none"><polyline points="20 6 9 17 4 12"/></svg>
                        </span>
                        {{ arch }}
                      </li>
            ...
            <div v-else class="tab-content">
              ...
              <div class="info-box">
                ...
                <div>
                  <p>Calculates SHA-256 hashes for all models without a <code>.civitai.info</code> sidecar, queries the
                    Civitai API, and downloads metadata + preview images in-place.</p>
            ...
            <button v-if="activeTab === 'sort'" class="primary-btn" @click="executeOrganization"
                    :disabled="isProcessing">
    ...
    <SummaryModal v-if="showReportModal && operationReport" :report="operationReport" :isDryRun="isDryRun"
                  @close="closeReport"/>
    <SettingsModal v-if="showSettings" :currentTheme="currentTheme" :availableThemes="THEMES" :isRecursive="isRecursive"
                   :isDryRun="isDryRun"
                   @applyTheme="applyTheme" @update:isRecursive="v => isRecursive = v"
                   @update:isDryRun="v => isDryRun = v" @close="showSettings = false"/>
