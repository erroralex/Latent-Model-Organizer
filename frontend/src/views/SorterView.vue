 ...
 * - Directory Management: Interfaces with the Electron host to pick folders via the system dialog.
 * - Architecture Selection: Provides a searchable, multi-select dropdown for filtering specific model types (e.g., SDXL, Flux, Pony).
 * - Recursive Logic: Toggles deep-scan capabilities for nested directory trees.
 * - Safety Measures: Includes a dry-run mode to preview changes without modifying the filesystem.
 *
 * @see App.vue
 * @see OrganizationService.java
 */
...
onMounted(() => { document.addEventListener('mousedown', onClickOutside); document.addEventListener('keydown', onKeydown); });
...
<template>
  <div class="tab-content">
    ...
    <InfoModal v-if="showInfo" title="Model Organizer Info" @close="showInfo = false">
      ...
      <p>Orphaned sidecar files (like preview images <code>.png</code>, <code>.json</code> metadata, and text files) are grouped with their parent model and moved together as an atomic unit to keep your library clean.</p>
    </InfoModal>
